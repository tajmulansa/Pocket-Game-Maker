...
class HtmlExporter {
  Future<HtmlExportResult?> exportProject(GameProject project) async {
    ...
    final root = await FilePicker.platform.getDirectoryPath(dialogTitle: 'Choose HTML5 export folder');
    ...
    await File('${out.path}${Platform.pathSeparator}index.html').writeAsString(_indexHtml(project));
    await File('${out.path}${Platform.pathSeparator}game.js').writeAsString(_gameJs(project, engine));
    final assetDir = Directory('${out.path}${Platform.pathSeparator}assets');
    await assetDir.create(recursive: true);
    for (final asset in project.assets) {
      await File('${assetDir.path}${Platform.pathSeparator}${asset.name}').writeAsBytes(asset.data, flush: true);
    }
  ...
  Future<Uint8List> buildZip(GameProject project) async {
    ...
    for (final asset in project.assets) archive.addFile(ArchiveFile('assets/${asset.name}', asset.data.length, asset.data));
    return Uint8List.fromList(ZipEncoder().encode(archive));
  }

  String buildPreviewHtml(GameProject project, String engine) => '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><title>${_esc(project.name)}</title><style>html,body{margin:0;background:#0b1020;overflow:hidden;width:100%;height:100%;display:grid;place-items:center}canvas{image-rendering:pixelated;max-width:100vw;max-height:100vh;background:#101827}</style></head><body><canvas id="game"></canvas><script>window.PGM_PROJECT=${jsonEncode(project.toJson())};</script><script>$engine</script></body></html>';

  String _indexHtml(GameProject project) => '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><title>${_esc(project.name)}</title><style>html,body{margin:0;background:#0b1020;overflow:hidden;width:100%;height:100%;display:grid;place-items:center}canvas{image-rendering:pixelated;max-width:100vw;max-height:100vh;background:#101827}</style></head><body><canvas id="game"></canvas><script src="game.js"></script></body></html>';

  String _gameJs(GameProject project, String engine) => 'window.PGM_PROJECT=${jsonEncode(project.toJson())};\n$engine';
